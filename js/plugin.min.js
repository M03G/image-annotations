(function ($) {
	...
	$(function () {
		...
		function ia_killanot(){
			...
			$(".anotBox").remove();
			$(".anotText").css({"display":"none"}).removeAttr("reply");
		}
		...
		function annot_edited(annot) {
			...
			annot.children('.ia-reply').removeClass('ia-di-none');
		}
		...
		$('.ia-annotation').on('click', '.ia-reply', function (e) {
			...
			var parent_annot = $(this).parent('li.ia-annotation');
			$(parent_annot.children('.ia-text')).after($(".anotText").attr("reply",parent_annot.attr("ia-id")));

			$(".anotText").css({"display":"block"})
		...
		$('body').on('click', '.new-annot-ok', function (e) {
			...
			var img = $(this).closest('article').attr('id');

			if ($(this).parent('.anotText').is("[reply]")) {
				var replyto_m = $(this).closest('.ia-annotation').attr("id").split('-');
				var replyto = replyto_m[1];
				var reply = $(this).parent('.anotText').attr("reply");
				query = "&reply=" + reply + "&img=" + img + '&replyto=' + replyto;
			} else {
				var box = $(".anotBox");
				...
				query = "&top=" + top + "&left=" + left + "&sidew=" + sidew + "&sideh=" + sideh +  "&img=" + img;
			}
		...
		$('.ia-list').on('click', '.ia-del', function (e) {
			if (confirm("Are you sure you want to delete the comment?")) {
				var del_annot = $(this).closest('.ia-annotation').attr("id").split('-');
				var delid = del_annot[1];
				...
				$.ajax({
					...
					success: function(data){
						if (data == "delok") {
							$('#annotation-' + delid).fadeOut(500).remove();
						}    					
					}
				});
			};
		...
		$('.ia-annotation').on('click', '.ia-edit', function (e) {
			...
			if (editann.hasClass('ia-edited')) {
				...
				editann.children('.ia-reply').addClass('ia-di-none');
		...
		$('.ia-list').on('click', '.edit-annot-ok', function (e) {
			var annot = $(this).closest('.ia-annotation');
			var newannot = annot.find('.ia-annot-edit').val();
			var nonce = annot.children('.ia-edit').attr('nonce');
			var replyto_annot = annot.attr("id").split('-');
			var annotid = replyto_annot[1];
		...
		$('.ia.ia-area > a').hover(			
			function(){
				var par = $(this).parent('.ia.ia-area');
				var coiid = par.attr("ia-id");
				...
				if (!$(".ia-hover-cloud").is("[ia-id = " + coiid + "]")) {
					...
					var comment_text = $('li#annotation-' + coiid + ' > .ia-text').text();
					var wi = 0;
					if (comment_text.length > 55) {
						wi = 55 * 6.5;
					} else {
						wi = comment_text.length * 6.5;
					}
					$(this).after('<div class="ia-hover-cloud" ia-id=' + coiid + 
						' style="top:' + (position_hover.top + position_height - 10) +
						'px;left: ' + (position_hover.left) + 'px;width:' + wi + 'px;"><div class="ia-hover-cloud-text"><strong style="color:'+color+';">' + 
						$('li#annotation-' + coiid + ' > .ia-author').html() + '</strong> ' +
		...
		$('.ia.ia-area > a').mouseleave(function(event) {
			...
			if ($(element_hover).attr("ia-id") == coiid && $(element_hover).hasClass("ia-hover-cloud")) {
				...
				$('.ia-hover-cloud[ia-id = ' + coiid + ']').mouseenter(function(event) {
					...
					if ($(element_enter).parent('.ia.ia-area').attr("ia-id") != coiid && !$(element_enter).parent('.ia.ia-area').hasClass("ia-area")) {
						$(this).remove();
				...
				$('.ia-hover-cloud[ia-id = ' + coiid + ']').mouseleave(function(event) {
					...
					if ($(element_out).parent('.ia.ia-area').attr("ia-id") != coiid) {
						$(this).remove();
						$('[ia-id = ' + coiid + ']').removeClass("hovered");
					}
				});
		...
		$('.ia-annotation').mouseover(function(e) {
			var coiid = $(this).attr("ia-id");
			e.stopPropagation();
			$(this).addClass("hovered");
			$('.ia-area[ia-id = ' + coiid + ']').addClass("hovered");
			$(this).children('.ia-edit').addClass("ia-b-hover");
			$(this).children('.ia-reply').addClass("ia-b-hover");
			$(this).children('.ia-del').addClass("ia-b-hover");
		}).mouseout(function(e) {
			var coiid = $(this).attr("ia-id");
			e.stopPropagation();
			$(this).removeClass("hovered");
			$('.ia-area[ia-id = ' + coiid + ']').removeClass("hovered");
			$(this).children('.ia-edit').removeClass("ia-b-hover");
			$(this).children('.ia-reply').removeClass("ia-b-hover");
			$(this).children('.ia-del').removeClass("ia-b-hover");
		})
